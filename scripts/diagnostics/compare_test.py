...
file_b = PROJECT_ROOT / "Data" / "Raport" / "new_08.xlsx"
...
B = B.fillna("")
A["key"] = (
    A["Name"].str.lower().str.replace(r"[^\w\s]", "", regex=True).str.strip()
    + "|"
    + A["Address"].str.lower().str.strip()
)
B["key"] = (
    B["Name"].str.lower().str.replace(r"[^\w\s]", "", regex=True).str.strip()
    + "|"
    + B["Address"].str.lower().str.strip()
)

A["emails_all"] = (
    A[["Email 1", "Email 2", "Email 3", "Other Emails"]]
    .astype(str)
    .agg(",".join, axis=1)
...
B["emails_all"] = (
    B[["Email 1", "Email 2", "Email 3", "Other Emails"]]
    .astype(str)
    .agg(",".join, axis=1)
    .str.replace(r",+", ",", regex=True)
    .str.strip(",")
)
A["emails_set"] = A["emails_all"].apply(
    lambda s: set([e.strip() for e in s.split(",") if e.strip()])
)
B["emails_set"] = B["emails_all"].apply(
    lambda s: set([e.strip() for e in s.split(",") if e.strip()])
)
...
out = PROJECT_ROOT / "Data" / "Raport" / "compare_test_vs_new_08.csv"
rows = []
    ...
    rows.append(
        {
            ...
            "name": a["Name"],
            "address": a["Address"],
            "emails_test": ",".join(sorted(a["emails_set"])),
            "emails_new08": ",".join(sorted(b["emails_set"])),
            "sources_test": a.get("Sources", ""),
            "sources_new08": b.get("Sources", ""),
        }
    )
